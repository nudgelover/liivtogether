...
<path
		...
		<div class="col-12 col-lg-9">
			<div class="row">
				...
				<div style="padding: 10px;" class="col-12 col-lg-8 ">
					<div class="myjoins">
						<span><img style="width: 20px;"
							src="https://cdn-icons-png.flaticon.com/512/9382/9382189.png">
							내 포인트리 현황</span>
				...
				<div class="col-12">
				<hr style="height: 4px; color: #424242 !important; margin: 1rem 0 0 0;">


				<c:forEach var="item" items="${MyPointreeList}">
				    <div class="MyPointreeBox">
				        <div>
						    <c:if test="${not empty item.gplace}">
							    <c:choose>
							        <c:when test="${item.gplace eq 'attend'}">이벤트(출석체크) </c:when>
							        <c:when test="${item.gplace eq 'card'}"> 이벤트(카드맞추기) </c:when>
							        <c:otherwise>
							            ${item.gplace}
							        </c:otherwise>
							    </c:choose>
							</c:if>
				            <c:if test="${not empty item.uplace}">
							    <c:choose>
							        <c:when test="${item.uplace eq 'D'}">기부 </c:when>
							        <c:otherwise>
							            ${item.gplace}
							        </c:otherwise>
							    </c:choose>
							</c:if>
					           </br><span style="font-size: small; font-weight: 200; color: gray; margin: 15px 0;">${item.rdate}</span>
				        </div>
				        <div>
				            <c:if test="${not empty item.gplace}"><span style="color: blue;">적립 </span><span style="font-weight: bold;"><fmt:formatNumber value="${item.mount}" pattern="#,##0"/></span></c:if>
				            <c:if test="${not empty item.uplace}"><span style="color: red;">사용 </span><span style="font-weight: bold;">-<fmt:formatNumber value="${item.mount}" pattern="#,##0"/></span></c:if>
				        </div>
				    </div>
				</c:forEach>
					</br></br>
