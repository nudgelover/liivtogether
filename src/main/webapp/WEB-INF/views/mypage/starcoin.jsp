...
<path
		...
		<div class="col-12 col-lg-9">
			<div class="row">
				...
				<c:forEach var="item" items="${MyStarcoinList}">
				    <div class="MyPointreeBox">
				        <div>
						    <c:if test="${not empty item.gplace}">
							    <c:choose>
							        <c:when test="${item.gplace eq 'D'}">기부참여리워드 </c:when>
							        <c:when test="${item.gplace eq 'V'}">봉사참여리워드 </c:when>
							        <c:otherwise>
							            ${item.gplace}
							        </c:otherwise>
							    </c:choose>
				            ...
				            <c:if test="${not empty item.uplace}">
							    <c:choose>
							        <c:when test="${item.uplace eq 'S'}">세미나결제 </c:when>
						            <c:when test="${item.uplace eq 'E'}">ESG# 상품결제 </c:when>
							        <c:otherwise>
							            ${item.gplace}
							        </c:otherwise>
							    </c:choose>
				        ...
				        <div>
				            <c:if test="${not empty item.gplace}"><span style="color: blue;">적립 </span><span style="font-weight: bold;">${item.mount}</span></c:if>
				            <c:if test="${not empty item.uplace}"><span style="color: red;">사용 </span><span style="font-weight: bold;">-${item.mount}</span></c:if>
				        </div>
				    </div>
				</c:forEach>
					</br></br>
