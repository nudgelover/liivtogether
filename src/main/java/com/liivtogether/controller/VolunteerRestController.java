...
public class VolunteerRestController {
	...
	@PostMapping("/volunteer/update-views")
	public Object updateviews(int id) throws Exception {
		int result = 0;
		try {
			volunteerService.updateViews(id);
			result = 1;
	...
	@PostMapping("/volunteer/check-likes")
	...
	@PostMapping("/volunteer/reg-like")
	...
	@PostMapping("/volunteer/update-like")
	...
	@GetMapping("/volunteer/order")
	public ResponseEntity<List<Volunteer>> order(@RequestParam String topicSmall, @RequestParam String currentDate,
			@RequestParam boolean includeClosed, @RequestParam String order,
			@RequestParam(required = false) Integer offset) {
		try {
			List<Volunteer> list = volunteerService.getVolunteerList(topicSmall, currentDate, includeClosed, order, offset);

			for (Volunteer volunteer : list) {
				LocalDate edate = LocalDate.parse(volunteer.getEdate());
	...
	@GetMapping("/volunteer/recently-viewed")
	public Volunteer detail(Integer id) throws Exception {
		log.info("여기 왔니?");
